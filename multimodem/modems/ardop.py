...
class ArdopModemBackend(ModemBackend):
    ...
    def _handle_command_line(self, line: str) -> None:
        ...
        elif line.startswith("PTT TRUE"):
            # Same shape as VARA's PTT ON/OFF (modems/vara.py): covers
            # both pre-connection keying and mid-session ARQ frames.
            # request_ptt/release_ptt tell an active CONNECTED session
            # apart from a fresh acquisition (see channel.py), so this
            # can't clobber the session's own reservation.
            #
            # NOTE: ARDOP's own host interface spec says PTT should
            # normally be generated by the TNC itself, for leader-tone/
            # handshake latency reasons. This branch only does anything
            # useful -- and is only safe to rely on -- if ARDOP is
            # configured with no PTT device of its own; see the
            # ptt_control notes in config.example.toml. ARDOP itself
            # doesn't check this return value -- it already decided to
            # transmit -- so a refusal means it's about to send audio
            # with no hardware PTT keyed. Log it loudly.
            if not self.channel.request_ptt(self.name):
                log.warning(
                    "%s: PTT TRUE refused, channel busy (%s) -- radio will not be "
                    "keyed for this transmission",
                    self.name, self.channel.owner,
                )
        elif line.startswith("PTT FALSE"):
